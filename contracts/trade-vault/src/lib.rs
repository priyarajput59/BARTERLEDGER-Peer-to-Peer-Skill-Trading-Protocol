...
pub trait ReputationLedgerInterface {
    fn record_completion(env: Env, trader_a: Address, trader_b: Address, trade_id: u64, both_confirmed: bool);
...
impl TradeVault {
    ...
    pub fn propose_trade(
        ...
        env.events().publish((EV_PROPOSED, party_a.clone()), (id, party_b.clone(), service_a));
    ...
    pub fn accept_trade(env: Env, party_b: Address, trade_id: u64) {
        ...
        if trade.party_b != party_b   { panic!("not the counterparty"); }
        if trade.status != TradeStatus::Proposed { panic!("trade not in Proposed state"); }
        if env.ledger().timestamp() > trade.deadline { panic!("trade deadline passed"); }
        ...
        let count: u32 = env.storage().instance().get(&DataKey::ActiveCount).unwrap_or(0u32);
        env.storage().instance().set(&DataKey::ActiveCount, &(count + 1));

        env.events().publish((EV_ACCEPTED, party_b.clone()), (trade_id,));
    ...
    pub fn confirm_delivery(env: Env, caller: Address, trade_id: u64) {
        ...
        if !is_a && !is_b { panic!("caller is not a party to this trade"); }

        // Prevent double-confirm
        if is_a && (trade.status == TradeStatus::ConfirmedA || trade.status == TradeStatus::Completed) {
            panic!("party A already confirmed");
        }
        if is_b && (trade.status == TradeStatus::ConfirmedB || trade.status == TradeStatus::Completed) {
        ...
        env.events().publish((EV_CONFIRMED, caller.clone()), (trade_id, is_a));

        if new_status == TradeStatus::Completed {
            env.events().publish((EV_COMPLETED, trade.party_a.clone()), (trade_id,));
    ...
    pub fn raise_dispute(env: Env, caller: Address, trade_id: u64, reason: String) {
        ...
        env.events().publish((EV_DISPUTED, caller.clone()), (trade_id, reason));
    }
    ...
    pub fn cancel_trade(env: Env, caller: Address, trade_id: u64) {
        ...
        if caller != trade.party_a { panic!("only proposer can cancel before acceptance"); }
        if trade.status != TradeStatus::Proposed { panic!("can only cancel Proposed trades"); }
    ...
    fn finalize_trade(env: &Env, trade: &Trade) {
        ...
        let count: u32 = env.storage().instance().get(&DataKey::ActiveCount).unwrap_or(1u32);
        env.storage().instance().set(&DataKey::ActiveCount, &count.saturating_sub(1));
    }
    ...
    fn push_user_trade(env: &Env, user: &Address, trade_id: u64) {
        ...
        env.storage().persistent().set(&DataKey::UserTrades(user.clone()), &list);
    }
    ...
    pub fn get_active_count(env: Env) -> u32 {
        env.storage().instance().get(&DataKey::ActiveCount).unwrap_or(0u32)
    }
